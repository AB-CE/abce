...
def read_parameters(parameters_file='simulation_parameters.csv', delimiter='\t', quotechar='"'):
    ...
    for line in reader:
        ...
        if 'name' not in keys:
            print(keys)
            raise SystemExit('No "name" column in ' + parameters_file)
...
class Simulation:
    ...
    def __init__(self, simulation_parameters):
        ...
        self._db = abce_db.Database(simulation_parameters['_path'], 'database')
        try:
    ...
    def _register_action_groups(self):
        ...
        self._action_groups['_advance_round_db'] = self._advance_round_db
    ...
    def run(self):
        ...
        self._action_list.append('_advance_round_db')
        ...
        try:
            for year in xrange(self.simulation_parameters['num_rounds']):
                ...
                for action in self._action_list:
                    ...
                    self.ask_each_agent_in('all', '_clearing__end_of_subround')
                    self._wait_for_agents()
        finally:
            ...
            while self._db.is_alive():
                time.sleep(0.05)
            abce_db.convert_to_csv(self.simulation_parameters['_path'], 'database')
    ...
    def build_agents(self, AgentClass,  number=None, group_name=None, agents_parameters=None):
        ...
        for idn in range(num_agents_this_group):
            agent = AgentClass(self.simulation_parameters, agents_parameters[idn], [idn, group_name, self.__command_addresse, self.trade_logging_mode])
    ...
    def build_agents_from_file(self, AgentClass, parameters_file=None, multiply=1, delimiter='\t', quotechar='"'):
        ...
        agents_parameters = []
        agent_file = csv.reader(open(parameters_file), delimiter=delimiter, quotechar=quotechar)
    ...
    def _write_description_file(self):
        ...
        description.write('Path: ' + str(self.simulation_parameters['_path']))
        description.write('\n')
        description.write('\n')
