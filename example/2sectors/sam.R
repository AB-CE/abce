...
sam_cell <- function(seller, buyer, cell='quantity') {
	ret <- trade_unified[which (trade_unified$seller==seller & trade_unified$buyer==buyer),c('round', cell)]
	ret
}
...
sam <- function (t=1) {
	cast(trade_unified[which(trade_unified$round == t),], seller + good ~ buyer, sum, margins=c("grand_row","grand_col"))
}
